...
# DO NOT FUCKING TOUCH ANYTHING WRITTEN 
# BELOW!!!!
#
...
# Separating first and last names and affiliation to own lists.
...
if DEBUG: print "First and last names and affiliation separated to own lists."
...
for j in range(len(namelist)):
    ...
    # Parsing last name:
    # Stripping whitespace from the beginning and end of a name.
    if DEBUG: print "Parsing last name %s, removing whitespace." % namelist_last[j]
    namelist_last[j] = namelist_last[j].strip()
    # Capitalizing first letter of string.
    if DEBUG: print "Parsing last name %s, capitilizing first letter" % namelist_last[j]
    ...
    for i in range(len(namelist_last[j])):
        if DEBUG: print "Parsing last name %s, checking for dashes or spaces at index %d." % (namelist_last[j],i)
        if (namelist_last[j][i] == "-" ) or (namelist_last[j][i] == " "):
            if DEBUG: print "Parsing last name %s, found dash or whitespace at %d, capitilizing letter at %d." % (namelist_last[j],i,i+1)
            namelist_last[j] = namelist_last[j][:i+1] + namelist_last[j][i+1].upper() + namelist_last[j][i+2:]
    if DEBUG: print "Nameparsing success!"

    # Parsing affiliation:
    if DEBUG: print "Parsing affiliation %s." % namelist_affiliation[j]
    # Capitalizing first letter of string.
    if DEBUG: print "Parsing affiliation %s, capitilizing first letter" % namelist_affiliation[j]
    namelist_affiliation[j] = namelist_affiliation[j].capitalize()
    # Checking for spaces to capitalize affiliation
    # (This part might be unstable for names like "Lars.droptable- Fuckswithyou---"")
    for i in range(len(namelist_affiliation[j])-1):
        if DEBUG: print "Parsing affiliation %s, checking for spaces at index %d." % (namelist_affiliation[j],i)
        if (namelist_affiliation[j][i] == " "):
            if DEBUG: print "Parsing affiliation %s, found whitespace at %d, capitilizing letter at %d." % (namelist_affiliation[j],i,i+1)
            namelist_affiliation[j] = namelist_affiliation[j][:i+1] + namelist_affiliation[j][i+1].upper() + namelist_affiliation[j][i+2:]
    if DEBUG: print "Nameparsing success!"
...
for j in range(len(namelist)):
    ...
    arranged_list_affiliation[int((j % pages)*TAGSPERPAGE +  math.floor(j / pages))] = namelist_affiliation[j]
...
for j in range(len(arranged_list_first)):
    line = OUTPUTPRETEXT + "{%s}{%s}{%s}\n" % (str(arranged_list_first[j]),str(arranged_list_last[j]),str(arranged_list_affiliation[j]))
